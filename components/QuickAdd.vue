<template>
  <div class="relative h-[580px] w-[450px] bg-[#C4C4C4] rounded-3xl flex flex-col p-6 mt-8 border-2 border-black">
    <h3 class="text-2xl font-semibold mb-4 text-center">QUICK ADD</h3>

    <form @submit.prevent="AddGoal">
      <div class="rounded-lg flex items-center justify-center text-neutral-500 select-none mb-5 mt-4">
        <Button @click="$event.target.closest('form').requestSubmit()" />
      </div>
      <h3 class="text-md font-semibold select-none mb-1">Goal</h3>
      <input v-model="goal_name"
        class="rounded-lg w-full border-2 border-neutral-800 px-3 py-2 text-sm text-gray-900 mb-2" type="text" required
        placeholder="Goal Name" />
      <h3 class="text-md font-semibold select-none mb-1">Description</h3>
      <input v-model="goal_desc"
        class="rounded-lg w-full border-2 border-neutral-800 px-3 py-2 text-sm text-gray-900 mb-2" type="text" required
        placeholder="Goal Description" />
      <h3 class="text-md font-semibold select-none mb-1">Deadline</h3>
      <input v-model="deadline" class="rounded-lg w-full border-2 border-neutral-800 px-3 py-2 text-sm text-gray-800"
