<template>
  <section class="flex flex-col items-center mt-8 px-6">
    <div
      class="relative h-[580px] w-[800px] bg-[#C4C4C4] rounded-3xl flex flex-col p-10 border-2 border-black">
      <div>
        <h3 class="text-2xl font-semibold mb-4 text-center">UPCOMING GOALS</h3>
        <ul class="space-y-4 max-h-[400px]">
          <li v-for="(goal, index) in upcomingGoals.slice(0, 5)" :key="index"
            class="flex justify-between items-center p-4 rounded-xl bg-[#1c2541] border-2 hover:scale-105 border-black duration-300">
            <div>
            ...
            <button
              class="px-4 py-2 rounded-lg text-sm font-medium bg-[#2963A5]/20 text-[#ffffff] filter hover:invert transition-all duration-200"
              @click="Completegoal(goal)">
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                <path fill="#2963A5" fill-rule="evenodd"
