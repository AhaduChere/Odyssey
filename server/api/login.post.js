...
export default defineEventHandler(async (event) => {
  ...
  await db.run("DELETE FROM Sessions WHERE user_id = ? AND created_at <= datetime('now', '-7 days')");

  await db.run('INSERT INTO Sessions (token, user_id) VALUES (?, ?)', token, user.user_id);
